...
const FeePaymentLog = (student_id) => {
  const [visibile, setvisibile] = useState(false);
  const [log, setlog] = useState([]);
  const handleclick = async () => {
    const { data, error } = await supabase
      .from("payment_log")
      .select("amount,date,remark,cashier")
      .eq("payedby", student_id.student_id)
      .order("id");
        ...
        <>
          {log.length === 0 ? (
            ...
            <>
              ...
              >
                PDF <img height={20} src={pdficon} alt="" />{" "}
              </Button>
            </>
          )}
        </>
